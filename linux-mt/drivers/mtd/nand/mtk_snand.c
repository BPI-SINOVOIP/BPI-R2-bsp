...
static const snand_flashdev_info gen_snand_FlashTable[] = {
	{{0xC8, 0xF4}, 2, 512, 128, 2048, 112, 0x0, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "GD5F4GQ4UAYIG", 0x00000003},
	{{0xEF, 0xAA}, 2, 128, 128, 2048, 64, 0x0, 0x00000000, 0x1A00001A,
		0x00000000, 0x0552000A, 0x01, "Winbond 1Gb", 0x00000000},
	{{0xC2, 0x22}, 2, 256, 128, 2048, 64, 0x0, 0x00000000, 0x1A00001A,
...
void mtk_snand_dev_unlock_all_blocks(struct mtk_snfc *snfc)
{
	...
	lock  = (cmd >> 16) & 0xFF;
	pr_warn("[SNF] Lock register(before):0x%x\n\r", lock);
...
static void mtk_snand_rs_reconfig_nfiecc(u32 row_addr)
{
	...
	if ((devinfo.advancedmode & SNAND_ADV_READ_SPLIT) == 0)
		return;
	pr_warn("row_addr:0x%x, g_snand_rs_num_page:0x%x\n", row_addr,
		g_snand_rs_num_page);

	if (row_addr < g_snand_rs_num_page) {
		pr_warn("mtk_snand_rs_reconfig_nfiecc , test01\n");
		if (g_snand_rs_cur_part != 0) {
			pr_warn("mtk_snand_rs_reconfig_nfiecc , test02\n");
		...
		pr_warn("mtk_snand_rs_reconfig_nfiecc , test03\n");
		if (g_snand_rs_cur_part != 1) {
			pr_warn("mtk_snand_rs_reconfig_nfiecc , test04\n");
				      ...
				      u32 u4PageAddr)
{
	...
	if ((u4ErrorNumDebug0 & 0x3F3F3F3F) != 0
	    || (u4ErrorNumDebug1 & 0x3F3F3F3F) != 0) {
		failed_sec = 0;
		...
		if (ret == 0)
			if (is_empty_page(spareBuf, sec_num)
			    ...
			    u4PageAddr)) {
				ret = 1;
				pr_warn("empty page have few filped bit(s)\n");
				      ...
				      u32 num_sec, u8 *buf)
{
	...
	if (g_bHwEcc) {
		...
		if (reg != 0) {
			...
			if (reg == 0x1F) {
				bRet = 0;
				pr_warn("ECC-U(2), PA=%d, S=%d\n",
					row_addr, (num_sec - 1));
			} else {
				if (reg)
					pr_warn("ECC-C(2), #err:%d, PA=%d, S=%d\n",
			      ...
			      struct mtk_snand_host *host)
{
	...
	dev_warn(dev, "bayi mtk_snand_init_hw");
