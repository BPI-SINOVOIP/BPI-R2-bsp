...
static int mtk_apmixedsys_probe(struct platform_device *pdev)
{
	struct clk_onecell_data *clk_data;
	int r;
	void __iomem *base;
...
static int mtk_topckgen_probe(struct platform_device *pdev)
{
	int r;
	void __iomem *base;
	struct device_node *node = pdev->dev.of_node;
...
static int mtk_mcucfg_probe(struct platform_device *pdev)
{
	int r;
	void __iomem *base;
	struct device_node *node = pdev->dev.of_node;
...
static const struct of_device_id of_match_clk_mt2712[] = {
	{
		...
		.data = mtk_apmixedsys_probe,
	}, {
		.compatible = "mediatek,mt2712-topckgen",
		.data = mtk_topckgen_probe,
	}, {
		.compatible = "mediatek,mt2712-infracfg",
		.data = mtk_infracfg_probe,
	}, {
		.compatible = "mediatek,mt2712-pericfg",
		.data = mtk_pericfg_probe,
	}, {
		.compatible = "mediatek,mt2712-mcucfg",
		.data = mtk_mcucfg_probe,
	}, {
		/* sentinel */
	}
};
