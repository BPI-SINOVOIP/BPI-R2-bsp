		...
		const u8 *event, const int event_len, u32 delay)
{
	...
	ret = btmtk_usb_meta_send_data(cmd, cmd_len);
	if (ret < 0)
		return ret;
	if (event != NULL && event_len > 0)
		...
		return ret;
	wait_event_timeout(p_wait_event_q, p_event_buffer != NULL, HZ);
	if (p_event_buffer == NULL) {
		BTUSB_ERR("%s: no event, timeout.", __func__);
		return -1;
	}
	...
	if (i != event_len) {
		BTUSB_ERR("%s: event not match.", __func__);
		ret = -1;
	}
...
void dump_hex(unsigned char *buf, int dump_len)
{
	...
	BTUSB_DBG_RAW(buf, dump_len, "%s: len: %d, Data:", __func__, dump_len);
...
static int btmtk_usb_BT_init(void)
{
	...
	if (ret) {
		BTUSB_ERR("fail to register_chrdev MAJOR:%d MINOR:%d, allocate chrdev again", MAJOR(devID), 0);
	...
	if (ret) {
		BTUSB_ERR("fail to register_chrdev MAJOR:%d MINOR:%d, allocate chrdev again", MAJOR(devID), 1);
	...
	if (ret) {
		BTUSB_ERR("fail to register_chrdev MAJOR:%d MINOR:%d, allocate chrdev again", MAJOR(devID), 2);
		ret = alloc_chrdev_region(&devIDfwlog, MAJOR(devID), 1, "BT_chrdev_raw");
		if (ret) {
			BTUSB_ERR("fail to allocate chrdev raw");
			return ret;
		}
	}
	...
	if (cdevErr)
		goto error;
		BTUSB_INFO("%s: %s driver(major %d, minor %d) installed.", __func__, "BT_chrdev",
	...
	if (cdevErr)
		goto error;
	BTUSB_INFO("%s: %s driver(major %d, minor %d) installed.", __func__, "BT_chrdevfwlog",
	...
	if (cdevErr)
		goto error;
	BTUSB_INFO("%s: %s driver(major %d, minor %d) installed.", __func__, "BT_cdev_raw",
	...
	if (IS_ERR(pBTDev_raw)) {
		BTUSB_ERR("device(stpbt_raw) create fail, error code(%ld)",
			PTR_ERR(pBTDev_raw));
		goto err2;
	}
...
static int btmtk_usb_send_wmt_power_on_cmd_7622(void)
{
	/*u8 count = 0;	 retry 3 times */
...
static int btmtk_usb_send_wmt_power_off_cmd_7622(void)
{
...
static void btmtk_bitf_rx_cb(const unsigned char *p_buf, unsigned int len)
{
	...
	dump_hex(buf, len);
	length = len;
	if (g_data->is_mt7668_dongle_state == BTMTK_USB_7668_DONGLE_STATE_POWERING_ON ||
		g_data->is_mt7668_dongle_state == BTMTK_USB_7668_DONGLE_STATE_POWERING_OFF) {
		p_event_buffer = kzalloc(length, GFP_KERNEL);
		memcpy(p_event_buffer, buf, length);
...
static int btmtk_get_buffer_size(void)
{
	int size;

	size = g_data->metabuffer->write_p - g_data->metabuffer->read_p;
	if (size < 0)
...
static ssize_t btmtk_usb_fops_read(struct file *file, char __user *buf, size_t count, loff_t *f_pos)
{
	...
	while (g_data->metabuffer->read_p != g_data->metabuffer->write_p) {
		if (g_data->metabuffer->write_p > g_data->metabuffer->read_p) {
			...
			copyLen += 6;
#endif
			if (copyLen > btmtk_get_buffer_size()) {
			...
			break;
				...
				g_data->metabuffer->read_p = (g_data->metabuffer->read_p + copyLen) % META_BUFFER_SIZE;
...
static int btmtk_usb_fops_open(struct inode *inode, struct file *file)
{
	...
	*/
	...
	msleep(20);
	btmtk_usb_send_wmt_power_on_cmd_7622();
...
#endif
	btmtk_usb_send_wmt_power_off_cmd_7622();
	msleep(20);
...
static int btmtk_fops_raw_open(struct inode *inode, struct file *file)
{
	...
	BTUSB_INFO("%s: Mediatek Bluetooth driver ver %s", __func__, VERSION);
