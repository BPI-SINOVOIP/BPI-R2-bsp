...
static void mtk_pcie_enable_port(struct mtk_pcie_port *port)
{
	...
	dev_info(dev, "pcie_enable_port debug 1 %d\n", port->slot);
	...
	dev_info(dev, "pcie_enable_port debug 2 %d\n", port->slot);
	...
	dev_info(dev, "pcie_enable_port debug 3 %d\n", port->slot);
	...
	dev_info(dev, "pcie_enable_port debug 4 %d\n", port->slot);
	...
	dev_info(dev, "pcie_enable_port debug 5 %d\n", port->slot);
	...
	dev_info(dev, "pcie_enable_port debug 6 %d\n", port->slot);
	...
	dev_info(dev, "pcie_enable_port debug 7 %d\n", port->slot);
	reset_control_assert(port->reset);
	dev_info(dev, "pcie_enable_port debug 8 %d\n", port->slot);
	reset_control_deassert(port->reset);

	dev_info(dev, "pcie_enable_port debug 9 %d\n", port->slot);
	...
	dev_info(dev, "pcie_enable_port debug 10 %d\n", port->slot);
	...
	dev_info(dev, "pcie_enable_port debug 11 %d\n", port->slot);
	if (!pcie->soc->startup(port)) {
		dev_info(dev, "pcie rc %d linkup success\n", port->slot);
		return;
	} else {
		dev_info(dev, "Port%d link up\n", port->slot);
	}

	dev_info(dev, "pcie_enable_port debug 12 %d\n", port->slot);
			       ...
			       int slot)
{
	...
	dev_err(dev, "mtk_pcie_parse_port %d debug 1\n", slot);
	...
	dev_err(dev, "mtk_pcie_parse_port %d debug 2\n", slot);
	...
	dev_err(dev, "mtk_pcie_parse_port %d debug 3\n", slot);
	...
	dev_err(dev, "mtk_pcie_parse_port %d debug 4\n", slot);
	...
	dev_err(dev, "mtk_pcie_parse_port %d debug 5\n", slot);
	...
	dev_err(dev, "mtk_pcie_parse_port %d debug 6\n", slot);
	snprintf(name, sizeof(name), "pcie-rst%d", slot);
	port->reset = devm_reset_control_get_optional(dev, name);
	if (PTR_ERR(port->reset) == -EPROBE_DEFER)
		return PTR_ERR(port->reset);

	dev_err(dev, "mtk_pcie_parse_port %d debug 7\n", slot);
	...
	dev_err(dev, "mtk_pcie_parse_port %d debug 8\n", slot);
	...
	dev_err(dev, "mtk_pcie_parse_port %d debug 9\n", slot);
	INIT_LIST_HEAD(&port->list);
	list_add_tail(&port->list, &pcie->ports);

	dev_err(dev, "mtk_pcie_parse_port %d debug 10\n", slot);
	return 0;
}
...
static int mtk_pcie_setup(struct mtk_pcie *pcie)
{
	...
	for_each_available_child_of_node(node, child) {
		...
		dev_err(dev, "parse pcie 1 slot : %d\n", slot);
		if (of_find_property(child, "mediatek,phy-switch", NULL)) {
			dev_err(dev, "change switch 1 : %d\n", slot);
			hif = syscon_regmap_lookup_by_phandle(child, "mediatek,phy-switch");
			if (IS_ERR(hif))
				return PTR_ERR(hif);

			dev_err(dev, "change switch 2 : %d\n", slot);
			regmap_update_bits(hif, HIF_SYSCFG1, HIF_SYSCFG1_PHY2_MASK, 0);
		}

		err = mtk_pcie_parse_port(pcie, child, slot);
		if (err)
			return err;
		dev_err(dev, "parse pcie slot : %d\n", slot);
		msleep(1000);
	}
