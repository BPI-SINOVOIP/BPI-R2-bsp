			 ...
			 struct sk_buff *skb, int gmac_no, int ring_no)
{
	...
	if (dbg == 1) {
		pr_err("M2Q_table[%d]=%d\n", skb->mark, M2Q_table[skb->mark]);
		pr_err("cpu_ptr->txd_info3.QID = %d\n", cpu_ptr->txd_info3.QID);
	}
	...
	if ((ei_local->features & FE_HW_VLAN_TX) && (lan_wan_separate == 1)) {
		if (web_sfq_enable == 1 && (skb->mark == 2)) {
			if (skb_vlan_tag_get(skb) == 1)
				cpu_ptr->txd_info3.QID = HW_SFQ_DL;
			else
				cpu_ptr->txd_info3.QID = HW_SFQ_UP;
		} else if (skb_vlan_tag_get(skb) == 2) {
			cpu_ptr->txd_info3.QID += 8;
		}
	}

	/* no hw van, no GE2, web UI used */
	if (!(ei_local->features & (FE_HW_VLAN_TX | FE_GE2_SUPPORT)) &&
	    (lan_wan_separate == 1)) {
...
void fe_qdma_rx_dma_deinit(struct net_device *dev)
{
	...
	for (i = 0; i < NUM_RX_DESC; i++) {
		skb_free_frag(ei_local->netrx_skb_data[0][i]);
...
int ei_qdma_xmit_housekeeping(struct net_device *netdev, int budget)
{
	...
	if (!(ei_local->features & FE_INT_NAPI)) {
		reg_val = sys_reg_read(QFE_INT_ENABLE);
		if (ei_local->features & FE_DLY_INT)
			sys_reg_write(QFE_INT_ENABLE, reg_val | RLS_DLY_INT);
		else
			sys_reg_write(QFE_INT_ENABLE, reg_val | RLS_DONE_INT);
	}
	return 0;
}
