	...
	unsigned int duration, u64 *cookie)
{
	...
	*cookie = ATOMIC_INC_RETURN(&pcfg80211_wdinfo->ro_ch_cookie_gen);
	...
	if (rtw_mi_check_status(padapter, MI_LINKED)) {
		...
		//Remove by Borg 2014/10/06 for Bug 45201^M
		//Roll back to original P2P listen setting to fix the issue that^M
		//P2P Sink is hard to be scanned by P2P Source.^M
		//else                                                  // p2p_listen, duration=5000^M
		//      duration = pwdinfo->ext_listen_interval  ^M
		//                   + (pwdinfo->ext_listen_interval/4);^M
	}
