...
/*
 * Description:
 *	Debug purpose: dump MAC register page 0/1
 *
 */
static void rtw_halmac_dump_mac_reg(struct dvobj_priv *d, const char* caller, char* status)
{
#ifdef DBG_POWER_SWITCH
	int i, j = 1;
	...
	for (i = 0x0; i < 0x100; i += 4) {
		if (j % 4 == 1)
			printk("0x%04x", i);
		printk(" 0x%08x ", rtw_read32(adapter, i));
		if ((j++) % 4 == 0)
			printk("\n");
	}
...
int rtw_halmac_poweron(struct dvobj_priv *d)
{
	...
	rtw_halmac_dump_mac_reg(d, __func__, "before calling power on");
	status = api->halmac_mac_power_switch(halmac, HALMAC_MAC_POWER_ON);
	rtw_halmac_dump_mac_reg(d, __func__, "after power on");
	if (HALMAC_RET_PWR_UNCHANGE == status) {
		...
		rtw_halmac_dump_mac_reg(d, __func__, "after power off and before calling power on");
		status = api->halmac_mac_power_switch(halmac, HALMAC_MAC_POWER_ON);
		rtw_halmac_dump_mac_reg(d, __func__, "after power on");
	...
	if (HALMAC_RET_SUCCESS != status) {
		if (HALMAC_RET_PWR_UNCHANGE == status) {
			printk("%s: HALMAC_RET_PWR_UNCHANGE (-2)\n",__func__);
			err = -2;
		} else
...
int rtw_halmac_poweroff(struct dvobj_priv *d)
{
	...
	rtw_halmac_dump_mac_reg(d, __func__, "before calling power off");
	status = api->halmac_mac_power_switch(halmac, HALMAC_MAC_POWER_OFF);
	rtw_halmac_dump_mac_reg(d, __func__, "after power off");
	if ((HALMAC_RET_SUCCESS != status)
