...
_func_enter_;
	...
	else
	{
		...
		if( pwdinfo->driver_interface == DRIVER_CFG80211 )
		{
			DBG_871X("[%s] Do nothing because buddy adapter is not linked", __FUNCTION__);
		}
		else
			...
			switch( OUI_Subtype )//OUI Subtype
			{
				...
				case P2P_INVIT_REQ:
				{
					...
					if (!tx) {
						...
						if(check_buddy_fwstate(padapter, _FW_LINKED) && (flags&P2P_INVITATION_FLAGS_PERSISTENT) ) {
#else
						if(check_buddy_fwstate(padapter, _FW_LINKED)) {
#endif
