...
)
{
	...
	pHalmac_adapter->hw_config_info.ac_oqt_size = HALMAC_OQT_ENTRY_AC_8821C;
	pHalmac_adapter->hw_config_info.non_ac_oqt_size = HALMAC_OQT_ENTRY_NOAC_8821C;
	pHalmac_adapter->hw_config_info.usb_txagg_num = HALMAC_BLK_DESC_NUM_8821C;
	...
	pHalmac_api->halmac_pinmux_get_func = halmac_pinmux_get_func_8821c;
	pHalmac_api->halmac_pinmux_set_func = halmac_pinmux_set_func_8821c;
	pHalmac_api->halmac_pinmux_free_func = halmac_pinmux_free_func_8821c;
	pHalmac_api->halmac_get_hw_value = halmac_get_hw_value_8821c;
	pHalmac_api->halmac_set_hw_value = halmac_set_hw_value_8821c;
	pHalmac_api->halmac_cfg_drv_info = halmac_cfg_drv_info_8821c;
	...
	if (pHalmac_adapter->halmac_interface == HALMAC_INTERFACE_SDIO) {
		...
		pHalmac_api->halmac_pcie_switch = halmac_pcie_switch_8821c_sdio;
		pHalmac_api->halmac_interface_integration_tuning = halmac_interface_integration_tuning_8821c_sdio;
		pHalmac_api->halmac_tx_allowed_sdio = halmac_tx_allowed_8821c_sdio;
		pHalmac_api->halmac_get_sdio_tx_addr = halmac_get_sdio_tx_addr_8821c_sdio;
		...
		pHalmac_api->halmac_pcie_switch = halmac_pcie_switch_8821c_usb;
		...
		pHalmac_api->halmac_pcie_switch = halmac_pcie_switch_8821c_pcie;
...
)
{
	...
	if (pHalmac_adapter->txff_allocation.rx_fifo_expanding_mode != HALMAC_RX_FIFO_EXPANDING_MODE_DISABLE)
		HALMAC_REG_WRITE_8(pHalmac_adapter, REG_RX_DRVINFO_SZ, HALMAC_RX_DESC_DUMMY_SIZE_MAX_8821C >> 3);
...
)
{
	...
	if (pHalmac_adapter->txff_allocation.la_mode == HALMAC_LA_MODE_DISABLE) {
		if (pHalmac_adapter->txff_allocation.rx_fifo_expanding_mode == HALMAC_RX_FIFO_EXPANDING_MODE_DISABLE) {
			...
			pHalmac_adapter->txff_allocation.tx_fifo_pg_num = HALMAC_TX_FIFO_SIZE_8821C >> HALMAC_TX_PAGE_SIZE_2_POWER_8821C;
			PLATFORM_MSG_PRINT(pDriver_adapter, HALMAC_MSG_INIT, HALMAC_DBG_TRACE, "[TRACE]rx_fifo_expanding_mode = %d not support\n", pHalmac_adapter->txff_allocation.rx_fifo_expanding_mode);
		}
	} else {
	...
	if (halmac_trx_mode == HALMAC_TRX_MODE_DELAY_LOOPBACK)
		pHalmac_adapter->txff_allocation.rsvd_pg_num += HALMAC_RSVD_DLLB_PGNUM_8821C;
	...
	pHalmac_adapter->txff_allocation.rsvd_h2c_static_info_pg_bndy = pHalmac_adapter->txff_allocation.rsvd_h2c_queue_pg_bndy - HALMAC_RSVD_H2C_STATICINFO_PGNUM_8821C;
	pHalmac_adapter->txff_allocation.rsvd_h2c_extra_info_pg_bndy = pHalmac_adapter->txff_allocation.rsvd_h2c_static_info_pg_bndy - HALMAC_RSVD_H2C_EXTRAINFO_PGNUM_8821C;
	...
	PLATFORM_MSG_PRINT(pDriver_adapter, HALMAC_MSG_INIT, HALMAC_DBG_TRACE, "[TRACE]Set FIFO page\n");
	...
	PLATFORM_MSG_PRINT(pDriver_adapter, HALMAC_MSG_INIT, HALMAC_DBG_TRACE, "[TRACE]Init LLT table\n");
