...
void p2p_concurrent_handler(_adapter	*padapter)
{
	...
	if (rtw_mi_check_status(padapter, MI_LINKED)) {
		...
		// bug43584 is fixed by Borg 2014/06/16
		// In 1195 semi-concurrent mode, buddy adapter is connected to the AP when receiving the P2P handshake packets.
		// At this time, this timer is set for returning to the AP's channel.
		// After that, the application will disconnect the AP connection.
		// Do nothing to prevent switch to wrong channel in cfg80211 architecture.
			if( pwdinfo->driver_interface == DRIVER_CFG80211 )
			{
				RTW_INFO("[%s] Do nothing because buddy adapter is not linked", __FUNCTION__);
			}
			else
#endif
			{//wext
		...
		if (action == ACT_PUBLIC_VENDOR
			...
			case P2P_INVIT_REQ: {
				...
				if (!tx) {
					#if defined(CONFIG_CONCURRENT_MODE) && defined(CONFIG_CFG80211_ONECHANNEL_UNDER_CONCURRENT)
#ifdef CONFIG_DHC_PATCH
					//bug 43387 fixed by Borg 2014/06/05
					//In semi-concurrent mode, status code of invitation join response should be decided by wpa_supplicant in cfg80211 architecture.
					//"no common channel" should just be applied when receiving the persistent invitation request.
					//Don't need to run rtw_change_p2pie_ch_list() to force the wpa_supplicant to reply the invitation response with "no common channel".
					if (rtw_mi_check_status(padapter, MI_LINKED) && padapter->registrypriv.full_ch_in_p2p_handshake == 0 && (flags&P2P_INVITATION_FLAGS_PERSISTENT) ) {
#else
					if (rtw_mi_check_status(padapter, MI_LINKED) && padapter->registrypriv.full_ch_in_p2p_handshake == 0) {
#endif
