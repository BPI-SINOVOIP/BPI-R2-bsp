...
)
{
	...
	while (cal < cal_num_8821A) {
		...
		case ODM_RF_PATH_A:
		{
			...
			odm_set_bb_reg(p_dm_odm, 0x82c, BIT(31), 0x0); /* [31] = 0 --> Page C */
			odm_set_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)path, 0x58, 0x7fe00,
				odm_get_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)path, 0x8, 0xffc00));
			...
			if ((get_bonding_type_8881A() == BOND_8881AM) && (p_dm_odm->ext_pa_5g) && (p_dm_odm->ext_lna_5g)) {
				odm_set_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)path, 0xdf, 0x00800, 0x1);
				odm_set_rf_reg(p_dm_odm, (enum odm_rf_radio_path_e)path, 0x56, 0x003e0, 0x1);
			}
			...
			ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0x978 = 0x%x\n", odm_get_bb_reg(p_dm_odm, 0x978, MASKDWORD)));
			...
			for (i = 0; i < rx_iqk_loop; i++) {
				if (p_dm_odm->support_interface == 1)
					if (i == 0) {
						if ((get_bonding_type_8881A() == BOND_8881AM) && (p_dm_odm->ext_pa_5g) && (p_dm_odm->ext_lna_5g))
							odm_write_4byte(p_dm_odm, 0xc8c, 0x28161800);  /* Good */
						else
							odm_write_4byte(p_dm_odm, 0xc8c, 0x28161100);
						ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0xc8c (i=0) = 0x%x\n", odm_get_bb_reg(p_dm_odm, 0xc8c, MASKDWORD)));
					} else {
						if ((get_bonding_type_8881A() == BOND_8881AM) && (p_dm_odm->ext_pa_5g) && (p_dm_odm->ext_lna_5g))
							odm_write_4byte(p_dm_odm, 0xc8c, 0x28160c00);
						else
							odm_write_4byte(p_dm_odm, 0xc8c, 0x28160d00);
						ODM_RT_TRACE(p_dm_odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("0xc8c = 0x%x\n", odm_get_bb_reg(p_dm_odm, 0xc8c, MASKDWORD)));
					}
				else
					odm_write_4byte(p_dm_odm, 0xc8c, 0x28160d00);
				...
				while (1) {
					...
					odm_set_rf_reg(p_dm_odm, ODM_RF_PATH_A, 0xdf, 0x00800, 0x0);
...
)
{
	...
	odm_set_bb_reg(p_dm_odm, 0xc10, 0x03ff0000, 0x0);
}
...
)
{
	...
	if (p_dm_odm->mp_mode) {   		/* (MP_DRIVER == 1) */
#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
#if (MP_DRIVER == 1)
