...
unsigned int OnProbeReq(_adapter *padapter, union recv_frame *precv_frame)
{
	...
	u8 wifi_test_chk_rate = 1;
        ...
        if(padapter->dbg_RX_print != 0)
        {
            u8 * mac_src = get_sa(pframe);
            printk("[In %s, p2p_state=%d, mcs_rate=%d,MAC=%x:%x]\n",__FUNCTION__, rtw_p2p_state(pwdinfo), pattrib->mcs_rate,mac_src[4],mac_src[5]);
        }
