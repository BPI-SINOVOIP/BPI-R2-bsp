...
void rtl8814_set_raid_cmd(PADAPTER padapter, u64 bitmap, u8 *arg, u8 bw)
{
	...
	if (pHalData->fw_ractrl == _TRUE) {
		...
		H2CCommand[2] = (bw & 0x3) | Get_RA_LDPC_8814(psta) | Get_VHT_ENI(0, psta->wireless_mode, bitmap);
...
void rtl8814_set_BcnEarly_C2H_Rpt_cmd(PADAPTER padapter, u8 enable)
{
	...
	SET_8814A_H2CCMD_PWRMODE_PARM_BYTE5(u1H2CSetPwrMode, 0);
	FillH2CCmd_8814(padapter, H2C_SET_PWR_MODE, sizeof(u1H2CSetPwrMode), u1H2CSetPwrMode);
}
#endif
#endif
...
#else
		RTW_INFO("[C2H], Get C2H_EXTEND message, SubID = %d. But this SubID is not handled by driver currently\n", Extend_c2hSubID);
#endif
		break;
