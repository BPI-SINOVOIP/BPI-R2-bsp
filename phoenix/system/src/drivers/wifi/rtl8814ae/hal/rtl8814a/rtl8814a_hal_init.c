...
)
{
	...
	u1Byte PowerTrackingType = ((PROMContent[EEPROM_TX_PWR_CALIBRATE_RATE_8812] & 0xC0) >> 6);

	if (!AutoloadFail) {
		if (GetRegPowerTrackingType(Adapter) != 64)
			pHalData->RfPowerTrackingType = GetRegPowerTrackingType(Adapter);
	} else
		pHalData->RfPowerTrackingType = 0;

	if (PowerTrackingType == 0 || PowerTrackingType == 1 || PowerTrackingType == 2 || PowerTrackingType == 3)
		pHalData->RfPowerTrackingType = PowerTrackingType;
	else
		pHalData->RfPowerTrackingType = 0;

	RTW_INFO("PowerTracking Type: 0x%2x\n", pHalData->RfPowerTrackingType);
}
...
void UpdateHalRAMask8814A(PADAPTER padapter, struct sta_info *psta, u32 mac_id, u8 rssi_level)
{
	...
	phydm_UpdateHalRAMask(&pHalData->odmpriv, psta->wireless_mode, pHalData->rf_type, bw, MimoPs_enable, disable_cck_rate, &ratr_bitmap_msb, &ratr_bitmap_lsb, rssi_level);
...
void init_hal_spec_8814a(_adapter *adapter)
{
	...
	hal_spec->port_num = HW_PORT_NUM_8814A;
	hal_spec->proto_cap = PROTO_CAP_8814A;
