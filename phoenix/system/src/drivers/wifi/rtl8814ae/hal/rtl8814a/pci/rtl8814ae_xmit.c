...
s32	rtl8814ae_init_xmit_priv(_adapter *padapter)
{
	s32	ret = _SUCCESS;
	...
	tasklet_init(&pxmitpriv->xmit_tasklet,
		     (void(*)(unsigned long))rtl8814ae_xmit_tasklet,
...
void	rtl8814ae_free_xmit_priv(_adapter *padapter)
{
	struct dvobj_priv	*pdvobjpriv = adapter_to_dvobj(padapter);
...
s32 rtl8814ae_enqueue_xmitbuf(struct rtw_tx_ring	*ring, struct xmit_buf *pxmitbuf)
{
	_irqL irqL;
...
/* [copy] from 8192ee get_txdesc_buf_addr() in rtl8192ee_xmit.c */
u16	get_txbd_idx_addr(u16 ff_hwaddr)
{

	u16	txbuf_desc_idx = REG_BEQ_TXBD_IDX_8814A;
	switch (ff_hwaddr) {
	case BK_QUEUE_INX:
		txbuf_desc_idx = REG_BKQ_TXBD_IDX_8814A;
		break;

	case BE_QUEUE_INX:
		txbuf_desc_idx = REG_BEQ_TXBD_IDX_8814A;
		break;

	case VI_QUEUE_INX:
		txbuf_desc_idx = REG_VIQ_TXBD_IDX_8814A;
		break;

	case VO_QUEUE_INX:
		txbuf_desc_idx = REG_VOQ_TXBD_IDX_8814A;
		break;

	case BCN_QUEUE_INX:
		txbuf_desc_idx = REG_BEQ_TXBD_IDX_8814A;   /* need check */
		break;
	...
	case MGT_QUEUE_INX:
		txbuf_desc_idx = REG_MGQ_TXBD_IDX_8814A;
		break;

	case HIGH_QUEUE_INX:
		txbuf_desc_idx = REG_HI0Q_TXBD_IDX_8814A;   /* need check */
		break;
...
/*
  ...
  */
static void fill_txdesc_own(_adapter *padapter, struct xmit_frame *pxmitframe, u8 *txdesc, u32 current_desc_idx)
{
	struct xmit_priv	*pxmitpriv = &padapter->xmitpriv;
	...
	buf_desc_debug("TX: enter %s(%d), queue_idx = %d, desc_idx = %x, txdesc = %x, pointer = 0x%08x, dma_startaddr = 0x%08x\n ", \
		__FUNCTION__, __LINE__, queue_idx, current_desc_idx, (u32)txdesc, \
		       rtw_read32(padapter, get_txbd_idx_addr(queue_idx)),
		       (u32)ring->dma);
	...
	if (queue_idx == BCN_QUEUE_INX) {
		...
		SET_TX_BUFF_DESC_OWN_8814A(txdesc, 1);
	...
	rtw_write16(padapter, get_txbd_idx_addr(queue_idx), next_desc_idx);
	buf_desc_debug("TX: exit %s(%d), after update h/w idx, pointer = 0x%08x\n", __FUNCTION__, __LINE__, rtw_read32(padapter, get_txbd_idx_addr(queue_idx)));
#if 0
...
static void update_txbufdesc(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz)
{
	...
	dma_addr_t		mapping;
	u32				i = 0;
	u16				seg_num = ((RTL8814AE_SEG_NUM == 0) ? 2 : ((RTL8814AE_SEG_NUM == 1) ? 4 : 8));
	u16				tx_page_size_reg = 0;
	u16				page_size_length = 0;
	HAL_DATA_TYPE	*pHalData = GET_HAL_DATA(padapter);

	tx_page_size_reg = 1; /* Page size of transmit packet buffer. */ /* 8814A Tx Page size is 128, but 92EE is 256, change it by ylb 20130403 */
	...
	/* -- Reset first element */
	SET_TX_BUFF_DESC_LEN_0_8814A(pmem, 0);
	SET_TX_BUFF_DESC_PSB_8814A(pmem, 0);
	SET_TX_BUFF_DESC_OWN_8814A(pmem, 0);
	...
	SET_TX_BUFF_DESC_LEN_0_8814A(pmem, TX_WIFI_INFO_SIZE);
	SET_TX_BUFF_DESC_PSB_8814A(pmem, page_size_length);
	SET_TX_BUFF_DESC_ADDR_LOW_0_8814A(pmem, mapping);
	...
	buf_desc_debug("TX:%s, Offset16(0x%08x)\n", __FUNCTION__, *((u32 *)(pmem + 16)));
	buf_desc_debug("TX:%s, Offset20(0x%08x)\n", __FUNCTION__, *((u32 *)(pmem + 20)));
	buf_desc_debug("TX:%s, Offset24(0x%08x)\n", __FUNCTION__, *((u32 *)(pmem + 24)));
	buf_desc_debug("TX:%s, Offset28(0x%08x)\n", __FUNCTION__, *((u32 *)(pmem + 28)));
...
static s32 update_txdesc(struct xmit_frame *pxmitframe, u8 *pmem, s32 sz)
{
	...
	* 8192EE_TODO	 */
	...
	SET_TX_DESC_OFFSET_8814A(ptxdesc, TXDESC_SIZE);

#ifdef CONFIG_TX_EARLY_MODE
	/* No tested yet for 8192ee */
	SET_TX_DESC_PKT_OFFSET_8814A(ptxdesc, 1);
	SET_TX_DESC_OFFSET_8814A(ptxdesc, TXDESC_SIZE + EARLY_MODE_INFO_SIZE);
#endif
	...
	/* RTW_INFO("qsel=%d\n", pattrib->qsel); //qsel=18=10010 */
	SET_TX_DESC_QUEUE_SEL_8814A(ptxdesc,  pattrib->qsel);

	/* RTW_INFO("qos_en=%d\n", pattrib->qos_en); // qos_en=0 */
	if (!pattrib->qos_en) {
	...
	if ((pxmitframe->frame_tag & 0x0f) == DATA_FRAMETAG) {
		...
		if ((pattrib->ether_type != 0x888e) &&
			...
			if (pattrib->ampdu_en == _TRUE) {
				...
				SET_TX_DESC_AMPDU_DENSITY_8814A(ptxdesc, pattrib->ampdu_spacing/*pHalData->AMPDUDensity*/);
			} else
		...
		SET_TX_DESC_MBSSID_8814A(ptxdesc, pattrib->mbssid);
	...
	SET_TX_DESC_TX_BUFFER_SIZE_8814A(ptxdesc, sz);
...
static u8 *get_txbufdesc(_adapter *padapter, u8 queue_index)
{
	...
	if (ring->qlen == (ring->entries - 1)) {
		RTW_INFO("No more TX buf desc@%d, ring->qlen = %d, ring->entries = %d\n",
			 queue_index, ring->qlen, ring->entries);
		return NULL;
	}
	...
	if ((GET_TX_BUFF_DESC_OWN_8814A(pdesc)) && (queue_index != BCN_QUEUE_INX)) {
		RTW_INFO("No more TX buf desc@%d, ring->idx = %d,idx = %d\n", queue_index, ring->idx, idx);
		return NULL;
	}
...
s32 rtw_dump_xframe(_adapter *padapter, struct xmit_frame *pxmitframe)
{
	...
	for (t = 0; t < pattrib->nr_frags; t++) {
		...
		ptxdesc = get_txbufdesc(GET_PRIMARY_ADAPTER(padapter), ff_hwaddr);
		...
		/* [thlin] In fact, ptxdesc  is pointer of tx buf desc. so ptxdesc is not used by update_txdesc(). */
		...
		update_txbufdesc(pxmitframe, ptxdesc, sz);

		if (pxmitbuf->buf_tag != XMITBUF_CMD)
			rtl8814ae_enqueue_xmitbuf(&ptx_ring[ff_hwaddr], pxmitbuf);

		pxmitbuf->len = sz + TX_WIFI_INFO_SIZE;
		w_sz = sz; /* w_sz is for write port used. pcie write port do nothing. */

		wmb();
		/* update TBD write pointer */
		fill_txdesc_own(padapter, pxmitframe, ptxdesc, current_desc_idx);

		_exit_critical(&pdvobjpriv->irq_th_lock, &irqL);
		/* rtw_write16(padapter, REG_PCIE_CTRL_REG, ffaddr2dma(ff_hwaddr)); */ /* wrtie queue's poll bit to kick start */
...
void rtl8814ae_xmitframe_resume(_adapter *padapter)
{
	...
	while (1) {
		if (RTW_CANNOT_RUN(padapter)) {
			RTW_INFO("rtl8814ae_xmitframe_resume => bDriverStopped or bSurpriseRemoved\n");
			break;
		}
...
s32 rtl8814ae_mgnt_xmit(_adapter *padapter, struct xmit_frame *pmgntframe)
{
 ...
 */
s32	rtl8814ae_hal_xmit(_adapter *padapter, struct xmit_frame *pxmitframe)
{
...
static void rtl8814ae_hostap_mgnt_xmit_cb(struct urb *urb)
{
#ifdef PLATFORM_LINUX
...
s32 rtl8814ae_hostap_mgnt_xmit_entry(_adapter *padapter, _pkt *pkt)
{
#ifdef PLATFORM_LINUX
	u16 fc;
	int rc, len, pipe;
