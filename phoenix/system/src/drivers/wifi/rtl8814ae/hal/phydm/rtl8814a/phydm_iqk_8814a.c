...
{	
	...
	ODM_SetRFReg(pDM_Odm, ODM_RF_PATH_C, 0x8f, bRFRegOffsetMask, 0x88001);
	ODM_SetRFReg(pDM_Odm, ODM_RF_PATH_D, 0x8f, bRFRegOffsetMask, 0x88001);
	...
	)
{
	...
	ODM_SetBBReg(pDM_Odm, 0x910, BIT(23)|BIT(22), 0x0);
	/*by YN*/
...
{	
	...
	for(idx = 0; idx <= 1; idx++){						// ii = 0:TXK , 1: RXK
		...
		for (Path = 0; Path <= 3; Path++) {
			ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, ("==========S%d IQK ==========\n", Path));
			cal_retry = 0;
			fail = TRUE;
			while (fail) {
				...
				if (idx == TX_IQK) {
					...
					if (*pDM_Odm->pBandType == ODM_BAND_2_4G) {
						ODM_SetRFReg(pDM_Odm, Path, 0xdf, BIT(11), 0x1);
						ODM_SetRFReg(pDM_Odm, Path, 0x56, 0xfffff, 0x51ce1);
						switch (Path) {
						case 0:
						case 1:
							ODM_Write4Byte(pDM_Odm, 0xeb0, 0x54775477);
							break;
						case 2:
							ODM_Write4Byte(pDM_Odm, 0x18b4, 0x54775477);
							break;
						case 3:
							ODM_Write4Byte(pDM_Odm, 0x1abc, 0x75400000);
							ODM_Write4Byte(pDM_Odm, 0x1ab4, 0x77777777);
							break;
						}
					}
					IQK_CMD = (0xf8000001|(9-*pDM_Odm->pBandWidth)<<8|(1<<(4+Path)));
					ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_TRACE, ("TXK_Trigger = 0x%x\n", IQK_CMD));
				...
				while (notready) {
					...
					delay_count++;
					if (delay_count >= 20) {
						ODM_RT_TRACE(pDM_Odm, ODM_COMP_CALIBRATION, ODM_DBG_LOUD, ("S%d IQK timeout!!!\n", Path));
						_IQK_ResetNCTL_8814A(pDM_Odm);
						break;
					}
				}
			...
			if(idx == RX_IQK){
				...
				if (*pDM_Odm->pBandType == ODM_BAND_2_4G)
					ODM_SetRFReg(pDM_Odm, Path, 0xdf, BIT(11), 0x0);
			}		
...
#if (DM_ODM_SUPPORT_TYPE & ODM_AP)
	_IQK_IQK_failReport_8814A(pDM_Odm);
#endif	
}
