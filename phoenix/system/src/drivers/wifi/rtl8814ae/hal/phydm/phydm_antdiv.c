	...
	)
{
	...
	if (pDM_Odm->AntDivType == S0S1_SW_ANTDIV) {
		...
		for (i=0; i<ODM_ASSOCIATE_ENTRY_NUM; i++)
			phydm_antdiv_reset_statistic(pDM_Odm, i);
#endif
	...
	)
{
	...
	pDM_FatTable->MainAnt_Sum[macid] = 0;
	pDM_FatTable->AuxAnt_Sum[macid] = 0;
	pDM_FatTable->MainAnt_Cnt[macid] = 0;
	pDM_FatTable->AuxAnt_Cnt[macid] = 0;
	pDM_FatTable->MainAnt_Sum_cck[macid] = 0;
	pDM_FatTable->AuxAnt_Sum_cck[macid] = 0;
	pDM_FatTable->MainAnt_Cnt_cck[macid] = 0;
	pDM_FatTable->AuxAnt_Cnt_cck[macid] = 0;
}
	...
	)
{
	...
	//OFDM HW AntDiv Parameters
	ODM_SetBBReg(pDM_Odm, 0x8D4 , 0x7FF, 0xA0); //thershold
	ODM_SetBBReg(pDM_Odm, 0x8D4 , 0x7FF000, 0x30); //bias
		...
		if ((pDM_SWAT_Table->try_flag == SWAW_STEP_DETERMINE) && (pDM_SWAT_Table->RSSI_Trying == 0)) {
			...
			for (i=0; i<ODM_ASSOCIATE_ENTRY_NUM; i++)
			{
				...
				if(IS_STA_VALID(pEntry))
				{
					...
					main_ant_sum = (u4Byte)pDM_FatTable->MainAnt_Sum[i] + (u4Byte)pDM_FatTable->MainAnt_Sum_cck[i];
					aux_ant_sum = (u4Byte)pDM_FatTable->AuxAnt_Sum[i] + (u4Byte)pDM_FatTable->AuxAnt_Sum_cck[i];
					main_ant_cnt = (u4Byte)pDM_FatTable->MainAnt_Cnt[i] + (u4Byte)pDM_FatTable->MainAnt_Cnt_cck[i];
					aux_ant_cnt = (u4Byte)pDM_FatTable->AuxAnt_Cnt[i] + (u4Byte)pDM_FatTable->AuxAnt_Cnt_cck[i];
					
					Main_RSSI = (main_ant_cnt != 0) ? (main_ant_sum /main_ant_cnt) : 0;
					Aux_RSSI = (aux_ant_cnt != 0) ? (  aux_ant_sum /aux_ant_cnt) : 0;
					
					if(pDM_FatTable->MainAnt_Cnt[i]<=1 && pDM_FatTable->MainAnt_Cnt_cck[i] >= 1)
						Main_RSSI=0;	
					
					if(pDM_FatTable->AuxAnt_Cnt[i]<=1 && pDM_FatTable->AuxAnt_Cnt_cck[i] >= 1)
						Aux_RSSI=0;
	...
	)
{
	...
	ODM_UpdateRxIdleAnt(pDM_Odm, MAIN_ANT);
	phydm_keep_RxAckAnt_By_TxAnt_time( pDM_Odm, 0); /* Timming issue: keep Rx ant after tx for ACK ( 5 x 3.2 mu = 16mu sec)*/
