	...
	)
{
	...
	if ((SUBFeeCnt > 0) || (MUBFeeCnt > 0)) {
		for (i = ODM_RF_PATH_A; i <= ODM_RF_PATH_B; i++) {
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)i, 0xEF, BIT19, 0x1); /*RF Mode table write enable*/
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)i, 0x33, 0xF, 3); /*Select RX mode*/
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)i, 0x3E, 0xfffff, 0x00036); /*Set Table data*/	
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)i, 0x3F, 0xfffff, 0x5AFCE); /*Set Table data*/
			ODM_SetRFReg(pDM_Odm, (ODM_RF_RADIO_PATH_E)i, 0xEF, BIT19, 0x0); /*RF Mode table write disable*/
		}
	}
