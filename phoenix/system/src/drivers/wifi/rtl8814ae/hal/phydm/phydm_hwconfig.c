...
/*		else if(ConfigType == CONFIG_BB_PHY_REG_PG)
	...
	if (pDM_Odm->SupportICType == ODM_RTL8821C)
	{
		if (ConfigType == CONFIG_BB_PHY_REG) {
			...
			if (pDM_Odm->default_rf_set_8821c == SWITCH_TO_BTG)
				AGC_DIFF_CONFIG_MP(8821C, BTG);
		} else if (ConfigType == CONFIG_BB_PHY_REG_PG) {
			READ_AND_CONFIG(8821C, _PHY_REG_PG);
		} else if (ConfigType == CONFIG_BB_AGC_TAB_DIFF) {
			if (pDM_Odm->current_rf_set_8821c == SWITCH_TO_BTG)
				AGC_DIFF_CONFIG_MP(8821C, BTG);
			else if (pDM_Odm->current_rf_set_8821c == SWITCH_TO_WLG)
...
#if (RTL8821C_SUPPORT == 1)
	if (pDM_Odm->SupportICType & ODM_RTL8821C) {
		if (pPhyStaRpt->pwdb >= 57)
			RxPower = pPhyStaRpt->pwdb - 100;
		else
			RxPower = pPhyStaRpt->pwdb - 102;
	}
#endif
