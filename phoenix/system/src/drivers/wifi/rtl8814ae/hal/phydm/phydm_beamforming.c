	...
	)
{
	...
	if ((iotpeer == HT_IOT_PEER_ATHEROS) && !(pMgntInfo->RegUsbSafetySwitch & RT_UsbSafetySwitch_skip_bfer_For_QCA) )
		{
		pEntry->HtBeamformCap = pHTInfo->HtBeamformCap & (~BEAMFORMING_HT_BEAMFORMEE_ENABLE);
		pEntry->VhtBeamformCap = pVHTInfo->VhtBeamformCap & (~BEAMFORMING_VHT_BEAMFORMEE_ENABLE);
	} else {	
	...
	if ((pEntry->WirelessMode & WIRELESS_MODE_AC_5G) || (pEntry->WirelessMode & WIRELESS_MODE_AC_24G)) {
		...
		if ((iotpeer == HT_IOT_PEER_ATHEROS) && !(pMgntInfo->RegUsbSafetySwitch & RT_UsbSafetySwitch_skip_bfer_For_QCA) )
			pEntry->CurBeamformVHT = pEntry->CurBeamformVHT & (~BEAMFORMING_VHT_BEAMFORMEE_ENABLE);
		else	
			pEntry->CurBeamformVHT = pEntry->CurBeamformVHT;
