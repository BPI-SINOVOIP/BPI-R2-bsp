...
sint recv_ucast_pn_decache(union recv_frame *precv_frame, struct stainfo_rxcache *prxcache)
{
	...
	if (pattrib->encrypt == _AES_) {
		...
		if (PN_LESS_CHK(pkt_pn, curr_pn) ||
		    PN_EQUAL_CHK(pkt_pn, curr_pn)) {
			/* return _FAIL; */
		} else {
			_rtw_memcpy(prxcache->iv[tid],
				    (pdata + pattrib->hdrlen),
...
static u8 is_the_same_ch(PADAPTER padapter, union recv_frame *precv_frame)
{
	struct mlme_ext_priv	*pmlmeext = &(padapter->mlmeextpriv);
	...
	unsigned char *pos;
	unsigned int size = 0;
	u8 ch = 0, offset = 0, res = _FALSE;

	u8 *pframe = precv_frame->u.hdr.rx_data;
	uint len = precv_frame->u.hdr.len;

	if (len > MAX_IE_SZ) {
		RTW_DBG("%s IE too long for survey event\n", __func__);
		goto exit;
	}

	if (GetFrameSubType(pframe) == WIFI_BEACON)
		offset = _BEACON_IE_OFFSET_;
	else if (GetFrameSubType(pframe) == WIFI_PROBERSP)
		offset = _PROBERSP_IE_OFFSET_;

	left = len - sizeof(struct rtw_ieee80211_hdr_3addr) - offset;
	pos = pframe + sizeof(struct rtw_ieee80211_hdr_3addr) + offset;

	if (rtw_ieee802_11_parse_elems(pos, left, &elems, 1) == ParseFailed) {
		RTW_DBG("%s parse_elems fail\n", __func__);
		goto exit;
	}

	if (elems.ds_params && elems.ds_params_len != 0) {
		_rtw_memcpy(&ch, elems.ds_params, sizeof(ch));
		if (ch == pmlmeext->cur_channel)
			res = _TRUE;
		else
			RTW_DBG("%s: ds_params: %d curl_channel: %d\n",
				 __func__, ch, pmlmeext->cur_channel);
	}
...
{
	/* struct mlme_priv *pmlmepriv = &adapter->mlmepriv; */
	u8 check_A1 = _FALSE, check_ch = _FALSE;
...

	{
		...
		if (psta) {
			check_ch = is_the_same_ch(padapter, precv_frame);
			check_A1 = _rtw_memcmp(adapter_mac_addr(padapter),
				GetAddr1Ptr(precv_frame->u.hdr.rx_data),
				ETH_ALEN);

			psta->sta_stats.rx_mgnt_pkts++;
			if (check_ch && GetFrameSubType(precv_frame->u.hdr.rx_data) == WIFI_BEACON)
				psta->sta_stats.rx_beacon_pkts++;
			else if (GetFrameSubType(precv_frame->u.hdr.rx_data) == WIFI_PROBEREQ)
				psta->sta_stats.rx_probereq_pkts++;
			else if (GetFrameSubType(precv_frame->u.hdr.rx_data) == WIFI_PROBERSP) {
				if (check_ch && check_A1)
...
sint validate_recv_data_frame(_adapter *adapter, union recv_frame *precv_frame)
{
	...
	} else if (MacAddr_isBcst(pattrib->ra)){
