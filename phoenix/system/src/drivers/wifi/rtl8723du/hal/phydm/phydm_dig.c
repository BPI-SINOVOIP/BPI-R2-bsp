	...
	if(pDM_Odm->SupportICType & ODM_IC_11N_SERIES)
	{
		...
		if(pDM_Odm->SupportICType != ODM_RTL8723D) {
			ODM_SetBBReg(pDM_Odm,
				     ODM_REG_DBG_RPT_11N, bMaskDWord, 0x208);
			FalseAlmCnt->edcca_flag =
				(BOOLEAN)ODM_GetBBReg(pDM_Odm,
						      ODM_REG_RPT_11N, BIT30);
		} else {
			FalseAlmCnt->edcca_flag = (BOOLEAN)ODM_GetBBReg(pDM_Odm,
						0x9a0, BIT29);
		}
