...
u16 rtw_read_usb3_phy(_adapter *adapter, u8 offset)
{
	u8 addr = {(offset&0x3f)|0x40};
	rtw_write8(adapter,0xf055, addr);
	return rtw_read16(adapter,0xf056);
}

void rtw_write_usb3_phy(_adapter *adapter, u8 offset, u16 value)
{
	u8 addr = {(offset&0x3f)|0x80};
	rtw_write16(adapter, 0xf056, value);
	rtw_write8(adapter, 0xf055, addr);
}
