...
#if 0 /* ToDo: check with RF */
	/* merge from win driver: hal_ReadAmplifierType_8814A() in hal8814AEfuse.c */
	if (IS_HARDWARE_TYPE_8814AE(Adapter)) {
		/* add by ylb 20121218, Fix 8194 load the wrong BB/RF param */
		/* 2015/01/05 MH 8814AU add the code will cause unstable throughput. */
		pHalData->TypeGPA = pHalData->ExternalPA_2G?0x55:0x0;	/* 2G PA */
		pHalData->TypeGLNA = pHalData->ExternalLNA_2G?0x55:0x0;	/* 2G LNA */
		pHalData->TypeAPA = pHalData->ExternalPA_5G?0x55:0x0;	/* 5G PA */
		pHalData->TypeALNA = pHalData->ExternalLNA_5G?0x55:0x0;	/* 5G LNA */
	}
#endif
...
u8 GetHalDefVar8814A(PADAPTER padapter, HAL_DEF_VARIABLE variable, void *pval)
{
	...
	switch (variable)
	{
		...
		case HAL_DEF_RX_DMA_SZ_WOW:
			 *((u32 *)pval) = MAX_RX_DMA_BUFFER_SIZE_8814A;
			break;
