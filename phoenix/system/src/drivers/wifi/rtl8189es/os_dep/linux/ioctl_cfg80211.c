...
//#endif //CONFIG_P2P
	...
	if (request->n_channels == 1) {
        ...
        } else if ((request->n_channels > 0) && (request->n_channels<= RTW_CHANNEL_SCAN_AMOUNT)) {
...
#ifdef CONFIG_DHC_PATCH
	/* Bug43180: + Janet 20140522 wpa_cli will send different listen-ch, should not only assign listen-ch in the first time */
	padapter->wdinfo.listen_channel = remain_ch; 
#endif
...
static void rtw_cfg80211_init_ht_capab_ex(_adapter *padapter, struct ieee80211_sta_ht_cap *ht_cap, 
#if (KERNEL_VERSION(4, 7, 0) <= LINUX_VERSION_CODE)
...
static void rtw_cfg80211_init_ht_capab(_adapter *padapter, struct ieee80211_sta_ht_cap *ht_cap, 
#if (KERNEL_VERSION(4, 7, 0) <= LINUX_VERSION_CODE)
                enum nl80211_band band, u8 rf_type)
#else
                enum ieee80211_band band, u8 rf_type)
