...
)
{
	...
	if (channel <= 14) {
		/* signal source */
		odm_set_bb_reg(p_dm, 0xcb0, (MASKBYTE2 | MASKLWORD), 0x704570);
		odm_set_bb_reg(p_dm, 0xeb0, (MASKBYTE2 | MASKLWORD), 0x704570);
		odm_set_bb_reg(p_dm, 0xcb4, MASKBYTE1, 0x45);
		odm_set_bb_reg(p_dm, 0xeb4, MASKBYTE1, 0x45);
	} else if (channel > 35) {
		odm_set_bb_reg(p_dm, 0xcb0, (MASKBYTE2 | MASKLWORD), 0x174517);
		odm_set_bb_reg(p_dm, 0xeb0, (MASKBYTE2 | MASKLWORD), 0x174517);
		odm_set_bb_reg(p_dm, 0xcb4, MASKBYTE1, 0x45);
		odm_set_bb_reg(p_dm, 0xeb4, MASKBYTE1, 0x45);
	} else
		return false;
	...
	/* from s0 or s1 */
	odm_set_bb_reg(p_dm, 0x1990, (BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0)), 0x30);
	odm_set_bb_reg(p_dm, 0x1990, (BIT(11) | BIT(10)), 0x3);

	/* input or output */
	odm_set_bb_reg(p_dm, 0x974, (BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0)), 0x3f);
	odm_set_bb_reg(p_dm, 0x974, (BIT(11) | BIT(10)), 0x3);

	/* delay 400ns for PAPE */
	odm_set_bb_reg(p_dm, 0x810, MASKBYTE3 | BIT(20) | BIT(21) | BIT(22) | BIT(23), 0x211);
	...
	/* inverse or not */
	odm_set_bb_reg(p_dm, 0xcbc, (BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0)), 0x0);
	odm_set_bb_reg(p_dm, 0xcbc, (BIT(11) | BIT(10)), 0x0);
	odm_set_bb_reg(p_dm, 0xebc, (BIT(5) | BIT(4) | BIT(3) | BIT(2) | BIT(1) | BIT(0)), 0x0);
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Update CCA parameters for Bcut (Pkt%d, Intf%d, RFE%d), row = %d, col = %d\n",
		__func__, p_dm->package_type, p_dm->support_interface, p_dm->rfe_type, row, col));
#endif
}
...
)
{
	...
	/* A-cut */
	reg82c = odm_get_bb_reg(p_dm, 0x82c, MASKDWORD);
	...
	odm_set_bb_reg(p_dm, 0x82c, MASKDWORD, reg82c);
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Update CCA parameters for Acut\n", __func__));
#endif
}
...
)
{
	...
	if ((p_dm->rx_ant_status == BB_PATH_A) || (p_dm->rx_ant_status == BB_PATH_B)) {
		...
		/* 838[23:20] = 7 */
		/* 838[27:24] = 7 */
		odm_set_bb_reg(p_dm, 0x838, 0x0ffffff0, 0x777678);
	} else {
		/* 838[7:4] = 3 */
		/* 838[11:8] = 3 */
		/* 838[15:12] = 6 */
		...
		/* 838[23:20] = 7 */
		/* 838[27:24] = 7 */
		odm_set_bb_reg(p_dm, 0x838, 0x0ffffff0, 0x776633);
	}
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Update CCA parameters for Acut\n", __func__));
#endif
}
...
)
{
	if (bandwidth == CHANNEL_WIDTH_40) {
		...
		odm_set_bb_reg(p_dm, 0x8f0, BIT(31), 0x0);
	} else if (bandwidth == CHANNEL_WIDTH_80) {
		/* RX DFIR for BW80 */
		odm_set_bb_reg(p_dm, 0x948, BIT(29) | BIT(28), 0x2);
		odm_set_bb_reg(p_dm, 0x94c, BIT(29) | BIT(28), 0x1);
		odm_set_bb_reg(p_dm, 0xc20, BIT(31), 0x0);
		odm_set_bb_reg(p_dm, 0x8f0, BIT(31), 0x1);
	} else {
	...
	/* PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("phydm_rxdfirpar_by_bw_8821c\n"));*/
}
...
)
{
	...
	if ((path > RF_PATH_A) || (hw_rate > 0x53)) {
		PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: unsupported path (%d)\n", __func__, path));
		return false;
	}
	...
	for (i = 0; i < 4; i++) {
		if (i != rate_offset)
			txagc_content = txagc_content | (config_phydm_read_txagc_8821c(p_dm, path, rate_idx + i) << (i << 3));
		else
			txagc_content = txagc_content | ((power_index & 0x3f) << (i << 3));
	}
	odm_set_bb_reg(p_dm, (offset_txagc[path] + rate_idx), MASKDWORD, txagc_content);
#else
	odm_write_1byte(p_dm, (offset_txagc[path] + hw_rate), (power_index & 0x3f));
#endif

	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: path-%d rate index 0x%x (0x%x) = 0x%x\n",
...
)
{
	...
	*/
	p_dm->rfe_type_21c = p_dm->rfe_type;
	p_dm->rfe_type = p_dm->rfe_type >> 3;

	/*2.4G default rf set with wlg or btg*/
	if (p_dm->rfe_type_21c == 2 || p_dm->rfe_type_21c == 4 || p_dm->rfe_type_21c == 7)
		p_dm->default_rf_set_8821c = SWITCH_TO_BTG;
	else if (p_dm->rfe_type_21c == 0 || p_dm->rfe_type_21c == 1 ||
		p_dm->rfe_type_21c == 3 || p_dm->rfe_type_21c == 5 ||
		p_dm->rfe_type_21c == 6)
		p_dm->default_rf_set_8821c = SWITCH_TO_WLG;
	else if (p_dm->rfe_type_21c == 0x22 || p_dm->rfe_type_21c == 0x24 ||
		p_dm->rfe_type_21c == 0x27 || p_dm->rfe_type_21c == 0x2a ||
		p_dm->rfe_type_21c == 0x2c || p_dm->rfe_type_21c == 0x2f) {
		p_dm->default_rf_set_8821c = SWITCH_TO_BTG;
		odm_cmn_info_init(p_dm, ODM_CMNINFO_PACKAGE_TYPE, 1);
	} else if (p_dm->rfe_type_21c == 0x20 || p_dm->rfe_type_21c == 0x21 ||
		p_dm->rfe_type_21c == 0x23 || p_dm->rfe_type_21c == 0x25 ||
		p_dm->rfe_type_21c == 0x26 || p_dm->rfe_type_21c == 0x28 ||
		p_dm->rfe_type_21c == 0x29 || p_dm->rfe_type_21c == 0x2b ||
		p_dm->rfe_type_21c == 0x2d || p_dm->rfe_type_21c == 0x2e) {
		p_dm->default_rf_set_8821c = SWITCH_TO_WLG;
		odm_cmn_info_init(p_dm, ODM_CMNINFO_PACKAGE_TYPE, 1);
	}

	if (p_dm->rfe_type_21c == 3 || p_dm->rfe_type_21c == 4 ||
		p_dm->rfe_type_21c == 0x23 || p_dm->rfe_type_21c == 0x24 ||
		p_dm->rfe_type_21c == 0x2b || p_dm->rfe_type_21c == 0x2c)
		p_dm->default_ant_num_8821c = SWITCH_TO_ANT2;
	else
		p_dm->default_ant_num_8821c = SWITCH_TO_ANT1;

	p_dm->is_init_hw_info_by_rfe = true;
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("%s: RFE type (%d), rf set (%s)\n",
		__FUNCTION__, p_dm->rfe_type_21c,
		p_dm->default_rf_set_8821c == 0 ? "BTG" : "WLG"));
#endif
}
...
)
{
	...
	odm_set_bb_reg(p_dm, 0x70, BIT(26), 0x1);
	...
	odm_set_bb_reg(p_dm, 0x1704, MASKLWORD, gnt_val);
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: RF-%d 0x%x = 0x%x, bit mask = 0x%x\n",
			__func__, path, reg_addr, readback_value, bit_mask));
...
)
{
	...
	if (bit_mask != RFREGOFFSETMASK) {
		data_original = config_phydm_read_rf_reg_8821c(p_dm, path, reg_addr, RFREGOFFSETMASK);

		/* Error handling. RF is disabled */
		if (config_phydm_read_rf_check_8821c(data_original) == false) {
			PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Write fail, RF is disable\n", __func__));
			return false;
		}
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: RF-%d 0x%x = 0x%x (original: 0x%x), bit mask = 0x%x\n",
...
)
{
	...
	if (p_dm->is_disable_phy_api) {
		PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: disable PHY API for debug!!\n", __func__));
		return true;
	}
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: path-%d rate index 0x%x = 0x%x\n", __func__, path, hw_rate, read_back_data));
...
)
{
	...
	rf_reg18 = config_phydm_read_rf_reg_8821c(p_dm, RF_PATH_A, 0x18, RFREGOFFSETMASK);
	...
	if (central_ch <= 14) {
		...
		if (p_dm->default_rf_set_8821c == SWITCH_TO_BTG)
			config_phydm_switch_rf_set_8821c(p_dm, SWITCH_TO_BTG);
		else if (p_dm->default_rf_set_8821c == SWITCH_TO_WLG)
			config_phydm_switch_rf_set_8821c(p_dm, SWITCH_TO_WLG);

#endif
		/*RF TXA_TANK LUT mode*/
		odm_set_rf_reg(p_dm, RF_PATH_A, 0xdf, BIT(6), 0x1);

		/*RF TXA_PA_TANK*/
		odm_set_rf_reg(p_dm, RF_PATH_A, 0x64, 0x0000f, 0xf);
...
#if (PHYDM_FW_API_FUNC_ENABLE_8821C == 1)
		/* Switch WLA */
		config_phydm_switch_rf_set_8821c(p_dm, SWITCH_TO_WLA);

#endif
		/*RF TXA_TANK LUT mode*/
		odm_set_rf_reg(p_dm, RF_PATH_A, 0xdf, BIT(6), 0x0);

	} else {
		PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Fail to switch band (ch: %d)\n", __func__, central_ch));
		return false;
	}

	odm_set_rf_reg(p_dm, RF_PATH_A, 0x18, RFREGOFFSETMASK, rf_reg18);

	if (phydm_rfe_8821c(p_dm, central_ch) == false)
		return false;

	if (rf_reg_status == false) {
		PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Fail to switch band (ch: %d), because writing RF register is fail\n", __func__, central_ch));
		return false;
	}

	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Success to switch band (ch: %d)\n", __func__, central_ch));
	return true;
}
...
)
{
	...
	if (p_dm->cut_version == ODM_CUT_A) {
		rf_reg_b8 = config_phydm_read_rf_reg_8821c(p_dm, RF_PATH_A, 0xb8, RFREGOFFSETMASK);
	...
	if (central_ch <= 14) {
		...
		odm_set_bb_reg(p_dm, 0xc1c, 0x00000F00, 0x0);
		p_dig_t->agc_table_idx = 0x0;
		...
		if (central_ch == 14) {
			odm_set_bb_reg(p_dm, 0xa24, MASKDWORD, 0x0000b81c);
			odm_set_bb_reg(p_dm, 0xa28, MASKLWORD, 0x0000);
			odm_set_bb_reg(p_dm, 0xaac, MASKDWORD, 0x00003667);
		} else {
			odm_set_bb_reg(p_dm, 0xa24, MASKDWORD, rega24_8821c);
			odm_set_bb_reg(p_dm, 0xa28, MASKLWORD, (rega28_8821c & MASKLWORD));
			odm_set_bb_reg(p_dm, 0xaac, MASKDWORD, regaac_8821c);
		}
		...
		if (central_ch >= 36 && central_ch <= 64)
			...
			PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Fail to switch channel (RF18) (ch: %d)\n", __func__, central_ch));
			return false;
		}

		/* 2. AGC table selection */
		if ((central_ch >= 36) && (central_ch <= 64)) {
			odm_set_bb_reg(p_dm, 0xc1c, 0x00000F00, 0x1);
			p_dig_t->agc_table_idx = 0x1;
		} else if ((central_ch >= 100) && (central_ch <= 144)) {
			odm_set_bb_reg(p_dm, 0xc1c, 0x00000F00, 0x2);
			p_dig_t->agc_table_idx = 0x2;
		} else if (central_ch >= 149) {
			odm_set_bb_reg(p_dm, 0xc1c, 0x00000F00, 0x3);
			p_dig_t->agc_table_idx = 0x3;
		} else {
			PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Fail to switch channel (AGC) (ch: %d)\n", __func__, central_ch));
			return false;
		}
		...
		if ((central_ch >= 36) && (central_ch <= 48))
			...
			PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Fail to switch channel (fc_area) (ch: %d)\n", __func__, central_ch));
			return false;
		}
		...
		if (central_ch == 153)
			phydm_csi_mask_setting(p_dm, FUNC_ENABLE, (u32)central_ch, 20, 5760, PHYDM_DONT_CARE);
		else if (central_ch == 151)
			phydm_csi_mask_setting(p_dm, FUNC_ENABLE, (u32)central_ch, 40, 5760, PHYDM_DONT_CARE);
		else if (central_ch == 155)
			phydm_csi_mask_setting(p_dm, FUNC_ENABLE, (u32)central_ch, 80, 5760, PHYDM_DONT_CARE);
		else
			phydm_csi_mask_setting(p_dm, FUNC_DISABLE, (u32)central_ch, 80, 5760, PHYDM_DONT_CARE);
#endif
	...
	if (p_dm->cut_version == ODM_CUT_A)
		odm_set_rf_reg(p_dm, RF_PATH_A, 0xb8, RFREGOFFSETMASK, rf_reg_b8);

	if (rf_reg_status == false) {
		PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Fail to switch channel (ch: %d), because writing RF register is fail\n", __func__, central_ch));
		return false;
	}

	phydm_ccapar_8821c(p_dm);
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Success to switch channel (ch: %d)\n", __func__, central_ch));
	return true;
}
...
)
{
	...
	if ((bandwidth >= CHANNEL_WIDTH_MAX) || ((bandwidth == CHANNEL_WIDTH_40) && (primary_ch_idx > 2)) || ((bandwidth == CHANNEL_WIDTH_80) && (primary_ch_idx > 4))) {
		PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Fail to switch bandwidth (bw: %d, primary ch: %d)\n", __func__, bandwidth, primary_ch_idx));
		return false;
	}
	/*Make protection*/
	if (central_ch_8821c == 165 && !(*p_dm->p_mp_mode))
...
#endif
		bb_reg8ac = odm_get_bb_reg(p_dm, 0x8ac, MASKDWORD);
		bb_reg8ac &= 0xffcffc00;
		bb_reg8ac |= 0x10010000;
		odm_set_bb_reg(p_dm, 0x8ac, MASKDWORD, bb_reg8ac);
...
#if 0
		odm_set_bb_reg(p_dm, 0x8ac, MASKBYTE0, (BIT(6) | CHANNEL_WIDTH_20));
	...
	if (rf_reg_status == false) {
		PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Fail to switch bandwidth (bw: %d, primary ch: %d), because writing RF register is fail\n", __func__, bandwidth, primary_ch_idx));
		return false;
	}

	/* Modify RX DFIR parameters */
	phydm_rxdfirpar_by_bw_8821c(p_dm, bandwidth);

	/* Modify CCA parameters */
	phydm_ccapar_by_bw_8821c(p_dm, bandwidth);
	phydm_ccapar_8821c(p_dm);

	/* Toggle RX path to avoid RX dead zone issue */
	/*odm_set_bb_reg(p_dm, 0x808, MASKBYTE0, 0x0);*/
	/*odm_set_bb_reg(p_dm, 0x808, MASKBYTE0, 0x11);*/



	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: Success to switch bandwidth (bw: %d, primary ch: %d)\n", __func__, bandwidth, primary_ch_idx));
	return true;
}
...
)
{
	...
	/* Switch band */
	if (config_phydm_switch_band_8821c(p_dm, central_ch) == false)
		return false;

	/* Switch channel */
	if (config_phydm_switch_channel_8821c(p_dm, central_ch) == false)
		return false;

	/* Switch bandwidth */
	if (config_phydm_switch_bandwidth_8821c(p_dm, primary_ch_idx, bandwidth) == false)
		return false;
...
)
{
	if (type == ODM_PRE_SETTING) {
		...
		rega24_8821c = odm_get_bb_reg(p_dm, 0xa24, MASKDWORD);
		rega28_8821c = odm_get_bb_reg(p_dm, 0xa28, MASKDWORD);
		regaac_8821c = odm_get_bb_reg(p_dm, 0xaac, MASKDWORD);
#if (PHYDM_FW_API_FUNC_ENABLE_8821C == 1)
	} else if (type == ODM_INIT_FW_SETTING) {
		u8	h2c_content[4] = {0};
		
		h2c_content[0] = p_dm->rfe_type_21c;
...
)
{
	...
	odm_set_bb_reg(p_dm, 0x1080, BIT(16), 0x1);
	odm_set_bb_reg(p_dm, 0x00, BIT(26), 0x1);
	/*odm_set_mac_reg(p_dm, 0x70, BIT(26), 0x1);*/
	/*odm_set_mac_reg(p_dm, 0x1704, MASKLWORD, 0x4000);*/
	/*odm_set_mac_reg(p_dm, 0x1700, (BIT(31) | BIT(30)), 0x3); */

	bb_reg32 = odm_get_bb_reg(p_dm, 0xcb8, MASKDWORD);
	switch (rf_set) {

	case SWITCH_TO_BTG:

		p_dm->current_rf_set_8821c = SWITCH_TO_BTG;

		bb_reg32 = (bb_reg32 | BIT(16));
		bb_reg32 &= (~(BIT(18) | BIT(20) | BIT(21) | BIT(22) | BIT(23)));
		odm_set_bb_reg(p_dm, 0xa84, MASKBYTE2, 0xe);
		odm_set_bb_reg(p_dm, 0xa80, MASKLWORD, 0xfc84);

#if (DM_ODM_SUPPORT_TYPE == ODM_WIN)
		if (*(p_dm->p_mp_mode) == true && p_mgnt_info->RegPHYParaFromFolder == 0)
#else
		if (*(p_dm->p_mp_mode) == true)
#endif
		{
			odm_set_bb_reg(p_dm, 0xaa8, 0x1f0000, 0x14);
			odm_config_bb_with_header_file(p_dm, CONFIG_BB_AGC_TAB_DIFF);
			/*Toggle initial gain twice for valid gain table*/
			odm_set_bb_reg(p_dm, ODM_REG(IGI_A, p_dm), ODM_BIT(IGI, p_dm), 0x22);
			odm_set_bb_reg(p_dm, ODM_REG(IGI_A, p_dm), ODM_BIT(IGI, p_dm), 0x20);
		}
		break;
	case SWITCH_TO_WLG:

		p_dm->current_rf_set_8821c = SWITCH_TO_WLG;

		bb_reg32 = (bb_reg32 | BIT(20) | BIT(21) | BIT(22));
		bb_reg32 &= (~(BIT(16) | BIT(18) | BIT(23)));
		odm_set_bb_reg(p_dm, 0xa84, MASKBYTE2, 0x12);
		odm_set_bb_reg(p_dm, 0xa80, MASKLWORD, 0x7532);
...
#endif
		{
			odm_set_bb_reg(p_dm, 0xaa8, 0x1f0000, 0x13);
			odm_config_bb_with_header_file(p_dm, CONFIG_BB_AGC_TAB_DIFF);
			/*Toggle initial gain twice for valid gain table*/
			odm_set_bb_reg(p_dm, ODM_REG(IGI_A, p_dm), ODM_BIT(IGI, p_dm), 0x22);
			odm_set_bb_reg(p_dm, ODM_REG(IGI_A, p_dm), ODM_BIT(IGI, p_dm), 0x20);
	...
	case SWITCH_TO_WLA:
		...
		p_dm->current_rf_set_8821c = SWITCH_TO_WLA;
	...
	case SWITCH_TO_BT:
		...
		p_dm->current_rf_set_8821c = SWITCH_TO_BT;
	...
	odm_set_bb_reg(p_dm, 0xcb8, MASKDWORD, bb_reg32);
#endif
}
...
)
{
	...
	p_dm->current_ant_num_8821c = ant_num;
	config_phydm_switch_rf_set_8821c(p_dm, rf_set);

	if (rf_set == SWITCH_TO_BT)
		phydm_set_gnt_state_8821c(p_dm, false, true); /* GNT_WL=0, GNT_BT=1 for BT test */
	else
		phydm_set_gnt_state_8821c(p_dm, true, false); /* GNT_WL=1, GNT_BT=0 for WL test */

	/*switch does not exist*/
	if (p_dm->rfe_type_21c == 0x5 || p_dm->rfe_type_21c == 0x6 ||
		p_dm->rfe_type_21c == 0x25 || p_dm->rfe_type_21c == 0x26 ||
		p_dm->rfe_type_21c == 0x2a || p_dm->rfe_type_21c == 0x2d ||
		p_dm->rfe_type_21c == 0x2e)
		return;

	if (p_dm->current_ant_num_8821c) /*Ant1 = 0, Ant2 = 1*/
	...
	odm_set_bb_reg(p_dm, 0xcb4, 0x000000ff, 0x77);
	...
	odm_set_bb_reg(p_dm, 0xcb4, 0x30000000, regval_0xcb7);
#endif
}
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: trx_capability = 0x%x\n", __func__, value32));
	return value32;
#else
	return 0;
#endif
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: stbc_capability = 0x%x\n", __func__, value32));
	return value32;
#else
	return 0;
#endif
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: ldpc_capability = 0x%x\n", __func__, value32));
	return value32;
#else
	return 0;
#endif
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: txbf_parameters = 0x%x\n", __func__, value32));
	return value32;
#else
	return 0;
#endif
...
)
{
	...
	PHYDM_DBG(p_dm, ODM_PHY_CONFIG, ("[%s]: txbf_capability = 0x%x\n", __func__, value32));
	return value32;
#else
	return 0;
#endif
...
{
#if (PHYDM_FW_API_FUNC_ENABLE_8821C == 1)
	return p_dm->default_rf_set_8821c;
#else
	return 0;
#endif
}
...
{
#if (PHYDM_FW_API_FUNC_ENABLE_8821C == 1)
	return p_dm->current_rf_set_8821c;
#else
	return 0;
#endif
}
...
{
#if (PHYDM_FW_API_FUNC_ENABLE_8821C == 1)
	u32				regval_0xcb4 = odm_get_bb_reg(p_dm, 0xcb4, BIT(29)|BIT(28));

	if (p_dm->current_rf_set_8821c == SWITCH_TO_BTG || p_dm->current_rf_set_8821c == SWITCH_TO_WLA || p_dm->current_rf_set_8821c == SWITCH_TO_BT) {
		if (regval_0xcb4 == 1)
			p_dm->current_ant_num_8821c = SWITCH_TO_ANT1;
		else if (regval_0xcb4 == 2)
			p_dm->current_ant_num_8821c = SWITCH_TO_ANT2;
	else
		if (regval_0xcb4 == 1)
			p_dm->current_ant_num_8821c = SWITCH_TO_ANT2;
		else if (regval_0xcb4 == 2)
	...
	return p_dm->current_ant_num_8821c;
#else
	return 0;
#endif
}
...
)
{
	...
	*/
	...
	if (p_dm->rfe_type_21c == 0 || p_dm->rfe_type_21c == 7 || p_dm->rfe_type_21c == 0x20 ||
		p_dm->rfe_type_21c == 0x27 || p_dm->rfe_type_21c == 0x28 || p_dm->rfe_type_21c == 0x2f)
		mapping_table = 1;
	else if (p_dm->rfe_type_21c == 1 || p_dm->rfe_type_21c == 2 || p_dm->rfe_type_21c == 0x21 ||
		p_dm->rfe_type_21c == 0x22 || p_dm->rfe_type_21c == 0x29 || p_dm->rfe_type_21c == 0x2a)
		mapping_table = 2;
	else if (p_dm->rfe_type_21c == 3 || p_dm->rfe_type_21c == 4 || p_dm->rfe_type_21c == 0x23 ||
		p_dm->rfe_type_21c == 0x24 || p_dm->rfe_type_21c == 0x2b || p_dm->rfe_type_21c == 0x2c)
		mapping_table = 3;
	else if (p_dm->rfe_type_21c == 5 || p_dm->rfe_type_21c == 6 || p_dm->rfe_type_21c == 0x25 ||
		p_dm->rfe_type_21c == 0x26 || p_dm->rfe_type_21c == 0x2d || p_dm->rfe_type_21c == 0x2e)
		mapping_table = 4;
