...
)
{
	...
	odm_config_rf_reg_8821c(p_dm, addr, data, RF_PATH_A, addr | maskfor_phy_set);
...
)
{
	...
	/*dbg_print("rf_gain_idx = 0x%x, p_dig_t->rf_gain_idx = 0x%x\n", rf_gain_idx, p_dig_t->rf_gain_idx);*/

	if (bb_gain_idx > 0x3c) {
		if ((rf_gain_idx == p_dig_t->rf_gain_idx) && (is_limit == false)) {
			is_limit = true;
			p_dig_t->big_jump_lmt[agc_table_idx] = bb_gain_idx - 2;
			PHYDM_DBG(p_dm, DBG_DIG, ("===> [AGC_TAB] big_jump_lmt [%d] = 0x%x\n", agc_table_idx, p_dig_t->big_jump_lmt[agc_table_idx]));
		}
	} else
		is_limit = false;

	p_dig_t->rf_gain_idx = rf_gain_idx;
...
{
	odm_update_agc_big_jump_lmt_8821c(p_dm, addr, data);
